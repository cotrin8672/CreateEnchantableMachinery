...
class EnchantableSpoutRenderer(
    ...
    override fun renderSafe(
        ...
        val fluidStack = primaryTank.renderedFluid
        var level = primaryTank.fluidLevel.getValue(partialTicks)

        val processingTicks = be.processingTicks
        val processingPT = processingTicks - partialTicks
        var processingProgress = 1 - (processingPT - 5) / 10
        ...
        if (!fluidStack.isEmpty && level != 0f) {
            val top = FluidVariantAttributes.isLighterThanAir(fluidStack.type)
